...

{
    ...
    if (GetGraphicDriver().IsNull())
    {
       ...
       //old version GetGraphicDriver() = Graphic3d::InitGraphicDriver(aDisplayConnection);
    }
...
void OccView::mouseMoveEvent( QMouseEvent * e )
{
    ...
    mView->Convert(e->pos().x(), e->pos().y(), x1, y1, z1);

    QString message = "x : " + QString::number(x1) + "   y: " + QString::number(y1);// + " z: " + QString::number(z1);

    emit MessageOfCoordinate(message);
}
...
void OccView::onLButtonDown( const int theFlags, const QPoint thePoint )
{
   ...
   /* if (state == 0) {
    ...
    if (state == 1) {
        ...
        mYmax = thePoint.y();
        V3d_Coordinate x1, y1, z1;
        mView->Convert(mXmax, mYmax, x1, y1, z1);

        mP2 = new gp_Pnt(x1, y1, z1);
        drawLine();
        state = 0;
    }*/
...
void OccView::panByMiddleButton( const QPoint& thePoint )
{
 ...
 void OccView::changeViewingPlane(int index)
{
 ...
 void OccView::initGrid(double xOrigin, double yOrigin, double xStep, double yStep, double xSize, double ySize)
 {
     //gp_Ax3 Ax3(aPlane.Location(), aPlane.Axis().Direction());
     //mViewer->SetPrivilegedPlane(Ax3);
     mViewer->SetRectangularGridValues(xOrigin, yOrigin, xStep, yStep, 0.0);
